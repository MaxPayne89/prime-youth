...
| Date | Tasks | Output |
|------|-------|--------|
| 2026-03-08 | T2, T3, T7 | T2: Identified redundant DB query in ParticipationHistoryLive (get_child_ids_for_parent called after get_children). T3: PR created (perf-assist/eliminate-redundant-child-query) — eliminates 1 extra SELECT per participation history mount. T7: Monthly summary updated. |
...
- T3 (Implement improvement): 2026-03-08
- T4 (Maintain PRs): 2026-03-07
- T5 (Comment on issues): 2026-03-07 (no perf issues open)
- T6 (Measurement infra): 2026-03-07
- T7 (Activity summary): 2026-03-08

## Optimization Backlog (prioritized)
1. **[MERGED] N+1 in DashboardLive** — PR #290 merged 2026-03-07.
2. **[MERGED] Missing LiveView telemetry** — PR #305 merged 2026-03-07.
3. **[IN REVIEW] Redundant DB query in ParticipationHistoryLive** — PR created 2026-03-08: `apply_history/4` called `get_child_ids_for_parent/1` redundantly after already having `children`; replaced with `MapSet.new(children, & &1.id)`.
...
5. **[MED] Parallelise program_detail_live.ex mount queries** — 3 sequential DB calls: get_program, load_team_members, get_participant_policy. Last 2 are independent — could use Task.async. Low urgency (detail page, not hot path).
...
## Backlog Cursor
- Next run: T1 (revalidate commands) or T4 (check new PR) + T6 (measurement infra) in next run
...
- `conversation_summaries` table already denormalizes `unread_count` and `latest_message_at` — backlog item #4 (conversation list) is already addressed at the data layer

## Active PRs
- `perf-assist/eliminate-redundant-child-query` — created 2026-03-08 (redundant get_child_ids_for_parent call in ParticipationHistoryLive)
