defmodule PrimeYouth.Auth.Adapters.Driven.Persistence.Repositories.UserRepositoryTest do
  ...
  describe "find_by_id/1" do
    test "returns user when id exists" do
      ...
      assert {:ok, found_user} = UserRepository.find_by_id(user.id)
      assert found_user.id == user.id
      assert found_user.email == user.email
    end

    test "returns error when id does not exist" do
      assert {:error, :not_found} = UserRepository.find_by_id(999_999)
    end
  end
  ...
  describe "find_by_email/1" do
    test "returns user when email exists" do
      ...
      assert {:ok, found_user} = UserRepository.find_by_email("test@example.com")
      assert found_user.id == user.id
  ...
  describe "update_email/2" do
    test "updates user email" do
      ...
      assert {:ok, updated_user} =
               UserRepository.update_email(domain_user, "newemail@example.com")
  ...
  describe "update_password/2" do
    test "updates user password" do
      ...
      assert {:ok, updated_user} =
               UserRepository.update_password(domain_user, new_hashed_password)
  ...
  describe "generate_session_token/1" do
    test "generates a session token for user" do
      ...
      assert {:ok, token} = UserRepository.generate_session_token(domain_user)
  ...
  describe "find_by_session_token/1" do
    test "returns user and timestamp when token is valid" do
      ...
      assert {:ok, {found_user, authenticated_at}} = UserRepository.find_by_session_token(token)
      assert found_user.id == user.id
      assert %DateTime{} = authenticated_at
    end

    test "returns error when token does not exist" do
      assert {:error, :not_found} = UserRepository.find_by_session_token("invalid_token")
    end
  end
  ...
  describe "delete_session_token/1" do
    test "deletes a session token" do
      ...
      assert :ok = UserRepository.delete_session_token(token)
      assert {:error, :not_found} = UserRepository.find_by_session_token(token)
    end
  end
  ...
  describe "generate_email_token/2" do
    test "generates confirmation email token" do
      ...
      assert {:ok, token} = UserRepository.generate_email_token(domain_user, :confirmation)
      assert is_binary(token)
    end
    ...
    test "generates magic link token" do
      ...
      assert {:ok, token} = UserRepository.generate_email_token(domain_user, :magic_link)
      assert is_binary(token)
    end
  end
  ...
  describe "verify_email_token/2" do
    test "verifies valid confirmation token" do
      user = insert(:user)
      {:ok, domain_user} = UserRepository.find_by_id(user.id)
      {:ok, token} = UserRepository.generate_email_token(domain_user, :confirmation)

      assert {:ok, verified_user} = UserRepository.verify_email_token(token, :confirmation)
      assert verified_user.id == user.id
    end

    test "returns error for invalid token" do
      assert {:error, :invalid_token} =
               UserRepository.verify_email_token("invalid_token", :confirmation)
    end
    ...
    test "returns error for wrong context" do
      ...
      assert {:error, :invalid_token} = UserRepository.verify_email_token(token, :magic_link)
    end
  end
end
