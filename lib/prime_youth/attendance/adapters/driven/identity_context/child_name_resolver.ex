defmodule PrimeYouth.Attendance.Adapters.Driven.IdentityContext.ChildNameResolver do
  @moduledoc """
  Adapter for resolving child names from Identity context.

  ## Anti-Corruption Layer

  This adapter serves as an anti-corruption layer between the Attendance and Identity
  bounded contexts. It:
  ...
  3. Maps Identity error semantics to Attendance error semantics

  ## Architecture

  ```
  Attendance Use Case → ForResolvingChildNames Port → [THIS ADAPTER] → Identity ChildRepository
       (uses string)        (behaviour contract)      (translates)         (owns Child model)
  ```

  The Attendance context never directly depends on `Identity.Domain.Models.Child`,
  ...
  Identity errors are mapped to Attendance semantics:

  - `:not_found` (Identity) → `:child_not_found` (Attendance)
  ...
  def resolve_child_name(child_id) when is_binary(child_id) do
    case child_repository().get_by_id(child_id) do
      ...
      {:error, :not_found} ->
        # Map Identity error semantics to Attendance semantics
