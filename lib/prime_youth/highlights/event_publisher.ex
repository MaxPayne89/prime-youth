defmodule PrimeYouth.Highlights.EventPublisher do
  @moduledoc """
  Convenience module for publishing Highlights context domain events.
  ...
  def publish_comment_added(%Post{} = post, author, comment_text, opts \\ []) do
    post
    |> HighlightsEvents.comment_added(author, comment_text, %{}, opts)
  ...
  def publish_post_liked(%Post{} = post, opts \\ []) do
    post
    |> HighlightsEvents.post_liked(%{}, opts)
    |> publisher_module().publish()
  end
  ...
  def publish_post_unliked(%Post{} = post, opts \\ []) do
    post
    |> HighlightsEvents.post_unliked(%{}, opts)
    |> publisher_module().publish()
  end
