defmodule PrimeYouthWeb.AttendanceComponents do
  @moduledoc """
  Provides attendance-specific components for Prime Youth application.

  This module contains domain-specific components for attendance check-in/out workflows,
  session management, and attendance status tracking for providers and parents.
  """
  use Phoenix.Component

  import PrimeYouthWeb.CoreComponents, only: [input: 1]
  import PrimeYouthWeb.UIComponents

  alias PrimeYouthWeb.Theme

  @doc """
  Renders a session attendance card with check-in status information.
  ...
  ## Examples
      ...
      <.attendance_card session={@session} role={:parent} />
  """
  attr :session, :map, required: true, doc: "Session map with attendance details"
  ...
  def attendance_card(assigns) do
    ~H"""
    <div class={[
      ...
      <div class="flex items-start justify-between gap-4 mb-4">
        ...
        <.attendance_status status={@session.status} />
      </div>
  ...
  @doc """
  Renders a status badge for sessions and attendance records.

  Displays color-coded status indicators with icons for various states including
  session statuses (scheduled, in_progress, completed) and attendance statuses
  (checked_in, checked_out, absent).

  ## Examples

      <.attendance_status status={:scheduled} />
      <.attendance_status status={:checked_in} size={:lg} />
  ...
  def attendance_status(assigns) do
    ~H"""
    <span class={[
  ...
  def check_in_form(assigns) do
    ...
    <div class={[
      ...
      <.form for={@form} id="attendance-form" phx-submit={@on_submit}>
        <%!-- Session context --%>
        <div class="mb-6">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              <label class="flex items-start gap-3 cursor-pointer">
                ...
                <.attendance_status status={record.status} size={:sm} />
              </label>
            </div>
          </div>
        </div>
  ...
  @doc """
  Renders a session roster list with attendance status.

  Displays all children enrolled in a session with their current attendance status.
      ...
      <div class="p-4 md:p-6 border-b border-gray-200">
        <div class="flex items-center justify-between">
          ...
          <div class="text-sm text-gray-600">
            {count_checked_in(@attendance_records)} / {length(@attendance_records)} checked in
          </div>
        </div>
      </div>
