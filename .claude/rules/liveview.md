...
1. Always set `phx-update="stream"` on the parent element, with a DOM id on the parent element like `id="messages"`
2. Consume the `@streams.stream_name` collection and use the id as the DOM id for each child
...
- LiveView streams are *not* enumerable, so you cannot use `Enum.filter/2` or `Enum.reject/2` on them
- To filter, prune, or refresh a list of items on the UI, you **must refetch the data and re-stream the entire stream collection, passing reset: true**:
...
- Come up with a step-by-step test plan that splits major test cases into small, isolated files. Start with simpler tests that verify content exists, gradually add interaction tests
...
When using changesets, the underlying data, form params, and errors are retrieved from it. The `:as` option is automatically computed too:
