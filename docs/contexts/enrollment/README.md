...
| Pricing | The enrollment total equals the program price set by the provider. Subtotal = total, VAT and card fees are zero. |
| Booking Usage | How many active enrollments a parent has in the current month vs. their subscription tier cap. |
| Payment Method | How the parent pays: `card` or `transfer`. |
...
- **No derived fees.** The enrollment total equals the program price. Subtotal = total, VAT and card fees are zero.
...
- [NEEDS INPUT] Are there refund implications when an enrollment is cancelled? No refund logic exists yet.
- [NEEDS INPUT] The pricing doesn't account for discounts, promo codes, or sibling discounts. Is this planned?
